...
- OpenAI API key: existing local project key reused without exposing its value. Authentication remains valid, but the provider returned an explicit credit-exhaustion 429 late in the latest rerun. At the operator's direction, no further paid evaluation calls are required for this MVP.
...
- Current local evidence: 181 tests passed with 6 provider-backed integration tests skipped; typecheck, lint, and the production prototype/runtime build passed. Earlier smoke evidence also retained Expo Doctor 20/20, API 200 responses from `/health` and `/ready` under a complete smoke configuration, and a 401 response from an unauthenticated application route.
- Android local evidence: the official API 36 toolchain and ARM64 emulator are installed; the universal debug APK built successfully and its empty-conversation, progress, assessment, animation, and authentication states were exercised natively. The new-conversation path now renders no seeded messages or assessment objects; Acme fixtures require an explicit non-production visual-QA state. Artifact hashes are retained in `docs/qa/native-android/README.md`. The emulator, Metro, and Gradle daemon are stopped after their test lanes.
...
- Recording gate: before any deployment announcement, retain a native video showing a real startup request, at least one domain-free follow-up turn, queued/running motion, persisted conversation restoration, terminal interactive objects, and inspectable evidence against the real API/Supabase/tool path. Fixture-only or visual-QA recordings are supplemental and cannot satisfy this gate.
- Remaining release work: retain the required signed-in native real API/Supabase/tool-path recording; install full Xcode plus an iOS Simulator runtime; then run native iOS E2E and physical-device acceptance. Only after those gates pass may a pinned container be published and deployed to Bluerose. No remote deployment occurs before those gates pass.
